...
export const api = {
  ...
  getGift: (eventId) => req('GET', `/gift/${eventId}`),
...
export const activateSuggestedEvent = async (id, suggested_amount) => {
  const token = localStorage.getItem('token');
  const res = await fetch(`${API_URL}/suggested-events/${id}/activate`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
    body: JSON.stringify({ suggested_amount })
  });
  return res.json();
};
